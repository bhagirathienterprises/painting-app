...
export async function generateInvoicePdf({ invoiceNo, customerName, customerType, customerPhone, workItems, subtotal, cgst, sgst, grandTotal, date }) {
  const { pdfDoc, page, font, fontBold, width, y: startY } = await createPdfWithHeader('SALES INVOICE', invoiceNo)
  let y = startY

  page.drawText('Billed To:', { x: 40, y, size: 10, font: fontBold, color: NAVY })
  y -= 15
  ...
  page.drawText(dateText, { x: width - 40 - font.widthOfTextAtSize(dateText, 10), y: y + 29, size: 10, font, color: GRAY })
